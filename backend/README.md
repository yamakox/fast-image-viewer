...
|WORKERS|main.py用。uvicornのワーカー数を指定する。|
|ALLOWED_HOSTS|Djangoサイトを提供するホスト名/ドメイン名を指定する。`;`で複数指定できる。|
|DEBUG|`True`の場合デバッグモードで起動する。本番では`False`にする。|
...
|FIV_APPDATA_FOLDER_PATH|`dataset.sqlite3`や`thumbnail.hdf5`などのアプリデータの格納先フォルダーを指定する。|
...
初めてバックエンドを使用する場合、またはテーブルに変更があった場合に実行する。
